...
import { addArrayMappingToTiles } from '../Services/Puzzle/initializer'
import {
  getAvailableActions,
  isPuzzleFinished
...
export default function PuzzleReducer(state = defaultState, action = {}) {
  switch (action.type) {
    case constants.INITIALIZE_PUZZLE:
      ...
      return {
        ...defaultState,
        puzzle: addArrayMappingToTiles(level.start, { name: 'click' }),
        level
      }
    case constants.CLICK_TILE:
      ...
      ] = Object.assign({ ...tileClicked }, { isActive: !tileClicked.isActive })
      return {
        ...state,
    ...
    case constants.TRIGGER_MOVE:
      ...
      const isFinished = isPuzzleFinished(currentPuzzle, state.level.end)
